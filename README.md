...
| 页面 | 网址 |
| --- | --- |
| 上海场 · 中文 | `https://www.taolinwei.com/Wedding-Invitation/` |
| 上海场 · English | `https://www.taolinwei.com/Wedding-Invitation/en/` |
| 内江场 · 中文 | `https://www.taolinwei.com/Wedding-Invitation/neijiang/` |

上海场的中英文两页在页尾互相有一个小链接，宾客点错了语言也能自己切过去。

**上海场和内江场之间没有任何链接**，这是刻意的：两场请的是两批宾客，
链接分别发给各自那批人，别让宾客看到另一场再犹豫该去哪一场。

## 页面内容

页面从上到下的顺序：

1. **婚礼请柬主视觉**：一张完整设计好的请柬图片（标题、新人姓名、
   日期、农历日期、签到合影/晚宴仪式时间、婚礼地址等信息都画在图里）。
   上海场是 `images/invitation.jpg`，内江场是 `images/invitation-neijiang.jpg`。
...
```
.
├── index.html          # 上海场中文请柬页，样式与交互均内嵌在此文件中
├── en/
│   ├── index.html      # 上海场英文请柬页（见下方"英文版"）
│   └── site.webmanifest
├── neijiang/
│   ├── index.html      # 内江场中文请柬页（见下方"内江场"）
│   └── site.webmanifest
├── admin.html          # 出席名单后台（口令进入，手工补录 + 导出 CSV）
├── favicon.ico
├── images
│   ├── invitation.jpg            # 上海场请柬主视觉图片
│   ├── invitation-neijiang.jpg   # 内江场请柬主视觉图片
...
├── tools/
│   └── lunar-check.py  # 核算请柬图上的农历日期（见下方"内江场"，跑一次就有结论）
└── README.md
```

> 部署时务必把 `images/` 文件夹和各页 HTML 一起上传/提交，漏传会导致图片 404
> （之前踩过这个坑）。
...
`neijiang/index.html`，网址 `https://www.taolinwei.com/Wedding-Invitation/neijiang/`。
四川的第二场酒席，只有中文版。

这一页是从 `index.html` 整份复制过来的——**同一套样式、同一段 JS、同一批照片和音乐**，
只换了三类东西：

| 换掉的 | 上海场 | 内江场 |
| --- | --- | --- |
| 请柬主视觉 | `images/invitation.jpg`（1111×1700） | `images/invitation-neijiang.jpg`（1014×1551） |
| 场地卡片 | 松江广富林希尔顿酒店会议中心 · 一楼大宴会厅<br>上海市松江区广轩路 1199 号<br>10 月 17 日（周六）17:08 签到 · 18:08 晚宴 | 内江万达嘉华酒店<br>四川省内江市东兴区汉安大道东段 999 号<br>10 月 24 日（周六）午宴 12:08 · 晚宴 18:08 |
| `<head>` 里的文案 | 描述/`og:url` 指向上海场 | 描述写内江万达嘉华酒店、`og:url` 指向 `/neijiang/`（标题两页**故意相同**，见下）|

改内江版要注意的几点：

- **所有资源路径都带 `../`**（因为它在 `neijiang/` 子目录里，和 `en/` 同理）。
  加图片时别忘了这个前缀，漏了就是 404。`neijiang/site.webmanifest` 里的图标路径同理
- **内江场是两场席**：午宴 12:08、晚宴 18:08。场地卡片那行写的是
  `午宴 12:08 · 晚宴 18:08`，不是上海场的"签到 / 晚宴"，别照抄过去
- **"复制地址"按钮复制的是 JS 里另一份常量**，在文件末尾 `var NAME` / `var ADDR`。
  改场地卡片里的文字时**必须同时改这两行**，否则卡片显示内江、复制出来是上海
- **页尾没有语言切换链接**（只有中文版），连 `.lang-switch` 那几条 CSS 也一起删了。
  以后要加英文版，从 `en/index.html` 抄这块回来
- **标题和上海场一字不差**，都是"陶林伟 & 肖佩韦 · 婚礼请柬"，`<title>`、`og:title`、
  `site.webmanifest` 的 `name` 三处都是。**这是有意的，不要给它加"（内江）"这类后缀**——
  标题是宾客在浏览器标签页和微信分享卡片上直接看到的东西，场次是给我们自己分辨的，
  没必要摊到宾客面前。两场靠 `description` 区分（一边写 10 月 17 日上海、一边写
  10 月 24 日内江），分享卡片上够清楚了
- `og:image` 仍然指向根目录那张 `share.jpg`（绝对地址，三个页面共用），
  所以内江场分享到微信，缩略图和上海场是同一张新人特写——这是有意的，那张图和场次无关

> **请柬图上的农历「丙午年九月十五」是核算过的，对**。
>
> 第一版图写的是"九月十四"，差了一天，找设计重出了一版（2026-08-29 换的图）。
> 核算依据：2026 年九月的朔（新月）在 **2026-10-10 23:50 北京时**，按定朔规则
> （含朔时刻的那一天为初一，以东八区为准，GB/T 33661-2017），九月初一 = 10-10，
> 所以 10-24 = 初一 + 14 天 = **九月十五**（顺带 10-17 = 九月初八，和上海场那张图对得上）。
>
> 之所以容易错：这次朔离 10-11 的日界只有 **10 分钟**，精度不够的日历库很容易把初一
> 算到 10-11 去，那样 10-24 就正好显示成"九月十四"。所以用了两种独立算法交叉验证——
> Meeus 第 49 章的朔级数、以及月球黄经（ELP 截断 59 项）与太阳黄经求合朔——
> 结果落在 23:49:53–23:50:38 之间（已涵盖含/不含光行差、两种 ΔT 取值的全部组合），
> 离日界的 10 分钟余量远大于秒级的算法误差。算法本身另用春节（2-17）、元宵（3-03）、
> 端午（6-19）、中秋（9-25）四个公开日期对过，全部吻合。
>
> **不想信算法的话有个一分钟旁证**：2026 年重阳节是 **10-18**。重阳就是九月初九，
> 所以九月初一 = 10-10，10-24 必然是十五。翻任何一本日历看重阳写哪天即可。
>
> 要重跑：`python3 tools/lunar-check.py`（只用标准库，无依赖）。
> 以后再换图、或者做第三场，农历都建议先用它核一遍——这种刀锋情形不算罕见。
...
- 用 CSS Grid 两列排布，每张图带一个比例类（三个页面各有一份同样的图片列表）：
  `r32`（横 3:2）、`r23`（竖 2:3），`full` 表示跨满两列
- **横幅照片通栏，竖幅照片两两成对**，两者交替出现，避免一路平铺下来；
  另外挑了几张竖幅做通栏当视觉重点
- 编号大致等于显示顺序（有跳号），想调整顺序改 `<button class="tile">` 的排列即可（三个页面都要改）
...
- 换了图但文件名不变时，**记得把三个页面里的 `?v=` 版本号加一**，
  否则宾客手机上的缓存会一直显示旧图（曾经踩过：部分人看到的是旧的模糊版）
- 点任意一张会打开灯箱大图，可用左右箭头/键盘方向键切换，点背景或按 Esc 关闭

所有图片长边压到 1600px、质量 0.88（上海场请柬图 1700px / 0.9，
内江场请柬图 1551px / 0.86，都是 350–380KB 这个量级）。
...
4. 几分钟后能在 `https://www.taolinwei.com/Wedding-Invitation/`、
   `/Wedding-Invitation/en/`、`/Wedding-Invitation/neijiang/` 三个地址打开
...
- 请柬视觉内容（标题、姓名、日期、地址等）都画在请柬图片里（上海场
  `images/invitation.jpg`，内江场 `images/invitation-neijiang.jpg`），
  要改这些内容需要重新出一张图替换掉对应的文件
- 走动动画、地址文字这些是各页 HTML 里的文本/代码，可以直接改，
  也可以把要改的内容发给我，由我代为修改后重新推送。
- 三个页面各自是一份独立的 HTML（没有共享的 CSS/JS 文件）。**改样式或改交互要三处都改**，
  只改一处的话另外两页会保持旧样子——这是当初为了让每页都能单文件打开而做的取舍
