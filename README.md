...
4. **图片展示区**：分三块——婚纱照、我们的日常（冰岛旅行 + 三只猫）、生活漫画，
   点任意一张可放大查看，支持左右切换和 Esc 关闭
5. **页尾**：新人姓名与日期
...
| 文件 | 内容 |
| --- | --- |
| `wedding-1..7.jpg` | 婚纱照（1、4 是横幅，其余竖幅） |
| `travel-1..2.jpg` | 冰岛旅行 |
| `cat-1..4.jpg` | 三只猫（英短蓝白 ×2、美短、金吉拉） |
| `comic-1..6.jpg` | 生活漫画 |

排版说明：

- 每块用 CSS Grid 两列排布，`index.html` 里每张图带一个比例类：
  `r32`（横 3:2）、`r23`（竖 2:3）、`r11`（正方形），`full` 表示跨满两列
- 婚纱照按"全幅 → 成对 → 全幅 → 成对 → 全幅"的节奏交替，中式婚纱照排在一起
- 正方形裁切用 `object-position:center 32%` 把主体往上带，避免把脸切掉
- 图片都带 `loading="lazy"`，往下滚动时才加载，首屏不会一次拉完
- 点任意一张会打开灯箱大图，可用左右箭头/键盘方向键切换，点背景或按 Esc 关闭

所有图片都已压缩过（长边 1000–1400px），整个 `images/` 目录约 3MB。
