...
- 缩略图是婚纱照裁出的脸部特写 `images/share.jpg`（400×400、约 24KB —— 图太大微信会直接跳过）
...
**需要你自己准备音频文件**，放到 `audio/bgm.mp3`（就叫这个名字），提交推送即可生效。
仓库里现在没有这个文件，此时按钮会自动保持隐藏，页面一切正常。

几点需要知道的：

- **浏览器禁止无交互自动播放带声音的媒体**，这是所有现代浏览器的硬性限制，绕不过去。
  所以做成了「访客第一次轻触页面任意处就开始播放」，之后可以用右上角按钮随时开关。
  微信内置浏览器另外走 `WeixinJSBridgeReady`，在微信里通常能直接自动播放
- 音量默认 45%，循环播放
- `preload="none"`：音频不会和请柬图抢带宽，只有真正要播时才下载
- 建议把 mp3 压到 3MB 以内（128kbps 左右足够），否则移动网络下载很慢

> **版权提醒**：如果你打算用《哈尔的移动城堡》的配乐（久石让《人生のメリーゴーランド》），
> 这是有版权的商业音乐。婚礼请柬这种小范围私人用途通常没人追究，但页面是公开可访问的，
> 严格来说属于灰色地带。介意的话可以找免版权（CC0 / Royalty-free）的钢琴曲替代。
